...
class EncoderRNN(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, layer_count, dropout_rate):
        ...
        self.rnn = nn.LSTM(embedding_dim, hidden_dim, num_layers=layer_count)
    ...
    def forward(self, input_data, src_field, device):
        ...
        return hidden, cell, packed_output

class DecoderRNN(nn.Module):
    def __init__(self, output_dim, embedding_dim, hidden_dim, layer_count, dropout_rate, AttentionMechanism, attention_params=None):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.layer_count = layer_count
        self.output_dim = output_dim
        #can consider arbitary attention mechanism
        if attention_params is None:
            self.attention = AttentionMechanism()
        else:
            self.attention = AttentionMechanism(*attention_params)
        
        self.embedding = nn.Embedding(output_dim, embedding_dim)
        self.dropout = nn.Dropout(dropout_rate)
        self.rnn = nn.LSTM(embedding_dim, hidden_dim, num_layers=layer_count)
        self.interim_layer = nn.Linear(2*hidden_dim + embedding_dim, hidden_dim)
        self.out_layer = nn.Linear(hidden_dim, output_dim)
    
    def forward(self, output_data, hidden_state, cell_state, encoder_hidden_outputs, mask):
        ##[input dims]
        ...
        context_state, _ = self.attention(output, encoder_hidden_outputs, mask)

        interim_input = torch.cat((output, context_state, embedding), dim=2).squeeze(0)
        hidden_interim = torch.tanh(self.interim_layer(interim_input))

        out_predict = self.out_layer(hidden_interim)
...
class DotProductAttention(nn.Module):
    ...
    #will return weighted context and softmax attention
    #weighted context dim : [1, batch_size, hidden_encoder_size]
    #softmax attention dim : [1, batch_size, seq_length]
    def forward(self, decoder_hidden, encoder_outputs, mask):
        #decoder_hidden = [1, batch size, dec_hid_dim]
        ...
        #repeat decoder hidden state src_len times for each batch
        hidden = decoder_hidden.repeat(src_len, 1, 1)
        
        #hidden = [src len, batch size, dec hid dim]
        #encoder_outputs = [src len, batch size, enc hid dim]
        
        #concat = [src_len, batch_size]
        attention = torch.sum(hidden * encoder_outputs, dim=-1).view(-1, batch_size, 1)
        
        #mask the zero padded encoder outputs
        attention = attention.masked_fill(mask==0, -1e10)
        
        #soft max attention= [src len, batch_size, 1]
        soft_max_attention = F.softmax(attention, dim=1) 
        
        scaled_encoder_outputs = encoder_outputs * soft_max_attention
        
        weighted_context = torch.sum(scaled_encoder_outputs, dim=0)
        
        return weighted_context.unsqueeze(0), soft_max_attention.T

class ConcatAttention(nn.Module):
    def __init__(self, encoder_hidden_dim, decoder_hidden_dim):
        super().__init__()
        self.encoder_dim = encoder_hidden_dim
        self.decoder_dim = decoder_hidden_dim
        self.out_layer = nn.Linear(encoder_hidden_dim+decoder_hidden_dim, 1)
    ...
    def forward(self, decoder_hidden, encoder_outputs, mask):
        ...
        #concat = [src_len, batch_size]
        attention = self.out_layer(torch.cat((hidden, encoder_outputs), dim=2))
...
class GeneralAttention(nn.Module):
    def __init__(self, encoder_hidden_dim, decoder_hidden_dim):
        ...
        self.encoder_dim = encoder_hidden_dim
        self.decoder_dim = decoder_hidden_dim
        self.out_layer = nn.Linear(encoder_hidden_dim, decoder_hidden_dim, bias=False)
    ...
    def forward(self, decoder_hidden, encoder_outputs, mask):
        ...
        #concat = [src_len, batch_size]
        attention = torch.sum(hidden * self.out_layer(encoder_outputs), dim=-1).view(-1, batch_size, 1)
...
class Seq2seqModel(nn.Module):
    ...
    def create_mask(self, encoder_seq_lengths):
        batch_size = len(encoder_seq_lengths)
        seq_length = torch.max(encoder_seq_lengths)
        mask = torch.zeros(batch_size, seq_length).to(self.device)
        for i in range(batch_size):
            mask[i, 0:encoder_seq_lengths[i]] = 1.0
    ...
    def forward(self, src_batch, trg_batch, src_field, teacher_verse=0.5):
        ...
        hidden, cell, packed_hidden_states = self.encoder(src_batch, src_field, self.device)
        
        #encoder_hidden_outputs dim = [seq_length, batch_size, 2*encoder_hidden_dim]
        #encoder_hidden_length dim = [batch_size]
        encoder_hidden_outputs, encoder_seq_lengths = pad_packed_sequence(packed_hidden_states)
        
        #creating mask
        mask = self.create_mask(encoder_seq_lengths)
        
        decoder_input = trg_batch[0, :] #<sos>
        
        for i in range(1, trg_len):
            
            decoder_output, hidden, cell = self.decoder(decoder_input, hidden, cell, encoder_hidden_outputs, mask)
...
def create_seq2seq_model(model_config, src_vocab, trg_vocab, device='cpu'):
    ...
    dec_attention_method = model_config['decoder']['attention_mechanism']
    dec_attention_args = model_config['decoder']['attention_args']

    enc = EncoderRNN(src_vocab, enc_emb_dim, enc_hid_dim, enc_layer_count, enc_dropout)
    dec = DecoderRNN(trg_vocab, dec_emb_dim, dec_hid_dim, dec_layer_count, dec_dropout, dec_attention_method, dec_attention_args)
...
def train_model(model, iterator, optimizer, loss_function, clip, src_field, trg_field):
    ...
    return epoch_loss / len(iterator), corpus_bleu([[x] for x in reference_sents], hypothesis_sents, smoothing_function=smoothie.method3)
...
def init(model_config, device='cpu'):
    ...
    if model_config['decoder'].get('attention_method_name', None) is not None:
        logging.critical('[CRITICAL] Using %s attention mechanism in deocder.' % (model_config['decoder']['attention_method_name']))
...
if __name__ == "__main__":
    ...
    model_config = {
        ...
        'decoder' : {
            ...
            'attention_method_name': "simple dot product",
            'attention_args' : None,  # [encoder_dim, decoder_dim]
        },
    }
    ...
    if not done_training:
        ...
        model_type = "effective_attention"
