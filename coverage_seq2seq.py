...
MODEL_NAME = 'linguistic-coverage-seq2seq'
CACHE_DIR = "/home/tushar/Desktop/MS/sem 2/nlpa/assignment-2/saved_models/coverage"
...
class EncoderRNN(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, layer_count, dropout_rate, bidirectional=True):
        super().__init__()
        ...
        self.rnn = nn.GRU(embedding_dim, hidden_dim, num_layers=layer_count, bidirectional=True)
    ...
    def forward(self, input_data, src_field, device):
        ...
        direc_hidden = hidden.view(self.layer_count, 2 if self.bidirectional else 1, batch_size, self.hidden_dim)
        #output dim = [seq_len, batch, hidden_dim]
        #hidden = [layer_count, batch, hidden_dim]
        
        return torch.cat((direc_hidden[:, 0], direc_hidden[:, 1]), dim=2), packed_output
...
class LinguisticCoverageInAttention(nn.Module):
    def __init__(self, enc_hid_dim, dec_hid_dim):
        super().__init__()
        
        self.attn = nn.Linear(enc_hid_dim + dec_hid_dim + 1, dec_hid_dim)
    ...
    def forward(self, decoder_hidden, encoder_outputs, coverage_vector, max_trg_len, mask):
        #decoder_hidden = [1, batch size, dec_hid_dim]
        #coverage_vector = [src_len, batch_size, 1]
        ...
        energy = torch.tanh(self.attn(torch.cat((hidden, encoder_outputs, coverage_vector/max_trg_len), dim = 2))) 
        
        #attention= [src len, batch_size, 1]
...
class Seq2seqModel(nn.Module):
    def __init__(self, encoder, decoder, device):
        ...
        self.attention = LinguisticCoverageInAttention(2*encoder.hidden_dim, decoder.hidden_dim)
        self.device = device
    ...
    def forward(self, src_batch, trg_batch, src_field, teacher_verse=0.5):
        ...
        encoder_context, packed_hidden_states = self.encoder(src_batch, src_field, self.device)
        ...
        #initializing coverage_vector
        coverage_vector = torch.zeros(encoder_hidden_outputs.shape[0], batch_size, 1).to(self.device)
        ...
        for i in range(1, trg_len):
            #attention_vector dim : [1, batch_size, seq_len]
            context_state, attention_vector = self.attention(hidden, encoder_hidden_outputs, coverage_vector, trg_len, mask)
            
            #update coverage_vector
            coverage_vector += attention_vector.T
...
if __name__ == "__main__":
    ...
    model_config = {
        ...
        'decoder' : {
            'emb_dim' : 128,
            'hidden_dim' : 512, #should be double of the encoder hidden_dim as written in paper
            'dropout' : 0.5,
            'layer_count' : 1,
        },
    }
